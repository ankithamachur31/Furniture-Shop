...
if (isset($_POST["confirm_order"])) {
    if (isset($_SESSION["customer"]) && isset($_SESSION["basket"]) && !empty($_SESSION["basket"])) {
        ...
        // Calculate the total and shipping cost
        $price = calculateTotal($_SESSION["basket"]);
        $shippingCost = 50;
        $price = $price + $shippingCost;

        // Loop through each item in the basket and insert into the orders table
        foreach ($_SESSION["basket"] as $item) {
            $prodId = $item["prodId"]; // Replace with the actual product ID from the cart
            $prodName = $item["prodName"]; // Replace with the actual product name from the cart
            $customerId=$item["customerId"];
            // Insert order information into the orders table
            $insertOrder = $conn->prepare("INSERT INTO orders (prodId, customerId, prodName, price) VALUES (?, ?, ?, ?)");
            if (!$insertOrder) {
                die("Prepare failed: (" . $conn->errno . ") " . $conn->error);
            }

            $insertOrder->bind_param("iisi", $prodId, $customerId, $prodName, $price);
            ...
            // Optionally, you can display a success message to the user for each item
            echo "Order for $prodName successfully placed!<br>";
        }

        // Optionally, you can display a final success message to the user
        echo "All orders successfully placed!";

        // Clear the session basket after the order is confirmed
...
// Function to calculate the total cost of the items in the basket
